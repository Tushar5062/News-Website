const API_KEY = '407270a6b5d1e6d875aef8dfaec52fb8'; 
const BASE_URL = 'https://gnews.io/api/v4';

async function fetchNews(query = '', topic = '') {
  const url = `${BASE_URL}/top-headlines?lang=en&country=in${
    query ? `&q=${query}` : ''
...
function displayNews(articles) {
  ...
  articles.forEach((article) => {
    ...
    newsCard.innerHTML = `
        <img src="${
          article.image ||
          'https://codewithtushar.com/tools/placeholder?size=300x200'
        }" alt="News Image">
        <div class="content">
          <h2>${article.title}</h2>
